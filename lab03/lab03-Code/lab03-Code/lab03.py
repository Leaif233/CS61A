...
def max_subseq(n, l):
    ...
    while n > 0:
        numbers.append(n % 10)
        n //= 10
    True_len = len(numbers) if l > len(numbers) else l
    def helper(site,val,n):
        if n == True_len:
            return val
        elif site == len(numbers):
            return 0 
        new_val = val+numbers[site]*pow(10,n)
        next1 = helper(site+1,new_val,n+1) 
        next2 = helper(site+1,val,n)
        return max(next1,next2)
    return 0 if l == 0 else helper(0,0,0)