...
def roll_dice(current_score, num_rolls, dice=six_sided):
    ...
    for i in range(num_rolls):
        ...
        if temp == 1:
            flag = True
        if  current_score % 2 == temp % 2:
            else_result += 1
        result += temp
    if flag:
        return 1 + else_result
    else:
...
def swine_swap(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    First = pow(3,player_score + opponent_score)
    Last = First % 10
    while First > 10:
        First //= 10
    if First == Last:
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            turn_score = take_turn(st0,score0,score1,dice,goal)
            score0 += turn_score
            if swine_swap(score0,score1):
                score0,score1 = score1,score0
        else:
            st1 = strategy1(score1,score0)
            turn_score = take_turn(st1,score1,score0,dice,goal)
            score1 += turn_score
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        if gain > running_high and gain > 0:
...
def max_scoring_num_rolls(current_score=0, dice=six_sided, trials_count=1000):
    ...
    for num in range(1, 11):
        ...
        current_average = averaged(current_score, num, dice)
...
def swine_swap_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    picky_score = picky_piggy(opponent_score)
    new_player_score = score + picky_score
    if swine_swap(new_player_score, opponent_score):
        swap_gain = opponent_score - score
        if swap_gain >= cutoff:
            return 0
    return num_rolls
    # END PROBLEM 11
