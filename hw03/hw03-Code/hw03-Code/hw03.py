...
def pingpong(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(site,dic,val):
        #base case:
        if site == n:
            return val
        return helper(site + 1, -1*dic, val-dic) if (number_of_six(site) > 0) or (site % 6 == 0) else helper(site + 1, dic, val+dic)
    return helper(1,1,1)
...
def missing_digits(n):
    ...
    last = n%10
    second_last = (n//10)%10
    exception = last - second_last - 1 if last != second_last else 0
    return exception + missing_digits(n//10)
...
def count_change(total, next_money):
    ...
    def helper(n,money):
        ...
        if n < 0:
            return 0
        try:
            change_1 = helper(n,next_money(money))
            change_2 = helper(n-money,money)
...
def multiadder(n):
    ...
    def helper(n,acc):
        if n == 0:
            return acc
        return lambda x:helper(n-1,acc+x)
    return helper(n,0)
